...
namespace MEWeb.Maintenance
{
  ...
  public class UsersVM : MEStatelessViewModel<UsersVM>
  {
        ...
        [Display(Name = "AccountType", Description = "")]
        [Singular.DataAnnotations.DropDownWeb(typeof(MELib.Account.AccountTypeList), UnselectedText = "Option", ValueMember = "AccountTypeID", DisplayMember = "AccountTypeName")]
   ...
   [WebCallable]
        public string GetUserInfo(int UserID)
        {
            try
            {
                var url = VirtualPathUtility.ToAbsolute("~/Account/AccountInfo.aspx?UserID=" + HttpUtility.UrlEncode((UserID.ToString())));
                return url;
            }
            catch (Exception e)
            {
