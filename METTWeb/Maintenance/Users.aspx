                ...
                var gridDivMain = rowDiv.Helpers.DivC("col-lg-9 paddingLeftRight0");
                {
                    var cardDiv = gridDivMain.Helpers.DivC("ibox float-e-margins");
                    {
                        ...
                        var cardContentDiv = cardDiv.Helpers.DivC("ibox-content");
                        {
                            ...
                            {
                                //var Account = cardContentDiv.Helpers.PagedGridFor<MELib.Accounts.Account>(c => c.UserListPageManager, c=> c.UserList, false, false);
                                ...
                                //var AccRow = Account.FirstRow;
                                {
                                    ...
                                    //AccRow.AddReadOnlyColumn(c=> c.Balance, 150);
                                    ...
                                    if (Singular.Security.Security.HasAccess("Security.Manage Users"))
                                    {
                                        ...
                                        var UserAccount = firstGridRow.AddColumn("Account Details");
                                        UserAccount.Style.Width = "150";
                                        UserAccount.Style.TextAlign = Singular.Web.TextAlign.center;
                                        var UserAccountBtn = UserAccount.Helpers.Button("User Info", Singular.Web.ButtonMainStyle.NoStyle, Singular.Web.ButtonSize.Normal, Singular.Web.FontAwesomeIcon.info_circle);
                                        UserAccountBtn.AddClass("btn-outline btn-info");
                                        UserAccountBtn.AddBinding(Singular.Web.KnockoutBindingString.click, "AccountInfo($data)");
                                    }
      ...
      if (ViewModel.EditingUser() != null) {
        ...
        var DeleteUser = function (User) {
			...
			ViewModel.CallServerMethod('CanDeleteUser', { UserID: User.UserID(), ShowLoadingBar: true }, function (result) {
                if (result.Success) {
                    window.location = '../Maintenance/Users.aspx';
					if (result.Data == true)
					{
        ...
        var AccountInfo = function (ROUser) {
            if (ROUser) {
                //Edit
                ViewModel.CallServerMethod('GetUserInfo', { UserID: ROUser.UserID(), ShowLoadingBar: true }, function (result) {
                    if (result.Success) {
